...
async function wrapper() {
    ...
    await prompt.get(prompt_schema, function (err, result) {
        if (err) {
            logs = ({
                data: err,
                type: "obj",
        ...
        if (result) {
            ...
            module_index.logging.main(logs);
            data.prompt_out = result;
            main(data);
        }
    });

    async function main(data) {
        data.user = await axios.get(`https://osu.ppy.sh/api/get_user?k=${api_key.osu_key}&u=${data.prompt_out.user_id}&m=${data.prompt_out.mode}&type=id`).then(res => {
            logs = ({
                data: res,
                type: "obj",
                level: 4
