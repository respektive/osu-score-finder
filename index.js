...
async function maps_get(){
    ...
    axios.get(`https://osu.ppy.sh/api/get_beatmaps?k=${config.osu_key}&m=${mode}&a=${include_converts}&since=${current_date}`).then(async res => {
        ...
        await fs.writeFile('maps.json', JSON.stringify(json), async function(data){ console.log(`Found ${map_count} Maps! Now checking scores...\n`); await score_get();})});
...
async function score_get(){
    ...
    await fs.readFile('data.json', function (err, data) {
        ...
        if(res.data[0] == null){
            ...
            res.data[0].beatmap_id = maps.maps[i].beatmap_id
            console.log(`Did not find a score on Beatmap: ${maps.maps[i].beatmap_id} - ${maps.maps[i].title} | ${maps.maps[i].version}!`)
        }else {
            res.data[0].beatmap_id = maps.maps[i].beatmap_id
            console.log(`Found a score on Beatmap: ${maps.maps[i].beatmap_id} - ${maps.maps[i].title} | ${maps.maps[i].version}! Rank: ${res.data[0].rank}`)
        }   

        json.scores.push(res.data[0])
        
        fs.writeFile("data.json", JSON.stringify(json), function(data){})
